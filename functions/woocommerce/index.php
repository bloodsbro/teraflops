...
function get_product_cart_html($product): string
{
  ...
  if (WC()->cart->find_product_in_cart($cart)) {
    ...
    return '
      <form style="display: contents" class="cart" action="' . esc_url(apply_filters('woocommerce_add_to_cart_form_action', $product->get_permalink())) . '" method="post" enctype="multipart/form-data">
        <input type="hidden" name="product_id" value="' . absint($product->get_id()) . '" />
        <input type="hidden" name="variation_id" class="variation_id" value="0" />
            
        <button type="submit" name="add-to-cart" value="' . esc_attr( $product->get_id() ) . '" data-product="' . $product->get_id() . '" class="btn btn-gradient add-product-cart single_add_to_cart_button">
          Додати у кошик
        </button>
      </form>
      ';
  ...
  ';
    ...
    $html .= '
      <form style="display: contents" class="cart" action="' . esc_url(apply_filters('woocommerce_add_to_cart_form_action', $product->get_permalink())) . '" method="post" enctype="multipart/form-data">
        ...
        <button type="submit" name="add-to-cart" value="' . esc_attr( $product->get_id() ) . '" data-product="' . $product->get_id() . '" class="btn btn-orange add-product-cart single_add_to_cart_button">
          Додати у кошик
        </button>
       </form>
  ';
...
function is_product_computer(): bool
{
  global $product;
...
function get_cross_products_html(): string
{
  global $product;
  $html = '';
  ...
  $html .= '<div class="product-body__item">
              <p class="product-item__title">
  ...
  foreach ($products->get_posts() as $productId) {
    $cross_sell = wc_get_product($productId);
    $secondImageUrl = get_field("hover-photo", $cross_sell->get_id()) ? get_field("hover-photo", $cross_sell->get_id())['url'] : '';
    $html .= '
            <div class="product-also__col">
              <div class="product">
                  <div class="product-img">
                    ' . $cross_sell->get_image(array(186, 186)) . '
                    <img class="hover-img" src="' . $secondImageUrl . '" alt="" width="186" height="186" />
                  </div>
                  <div class="product-body">
                      <p class="product-body__name">
                      ...
                      <div class="product-body__info">
                          <p class="product-body__price">
                             ' . $cross_sell->get_price_html() . '
                          </p>
                      </div>
                      <div class="product-body__bottom">
                          ' . do_shortcode("[yith_wcwl_add_to_wishlist]") . '
                          ' . get_product_cart_html($cross_sell) . '
                      </div>
                  </div>
              </div>
          </div>
...
function get_installment_html(): string
{
  if (is_product_computer()) {
    return '<p>Монтаж: <img src="' . get_template_directory_uri() . '/assets/img/install-ico.svg" alt=""></p>';
  }
          ...
          <div class="product-envelope__left">
              ...
              <div class="product-headings mobile">
                  ...
                  <div class="product-headings__detail">
                      ...
                      ' . get_installment_html() . '
                      <div style="display: flex;align-items: center;font-size: 20px;">Поділитися: ' . do_shortcode("[addtoany]") . '</div>
